...
export async function generateMetadata({ params }: NewsDetailParams) {
  ...
  const description = getLocalizedNewsValue(article, locale, "excerpt");
...
export default async function NewsDetailPage({ params }: NewsDetailParams) {
  ...
  const title = getLocalizedNewsValue(article, locale, "title");
  const excerpt = getLocalizedNewsValue(article, locale, "excerpt");
  const body = getLocalizedNewsBody(article, locale);
  const dateInfo = formatNewsDate(article.publishedAt, locale);
  const categoryLabel = getNewsCategoryLabel(article.category, locale);
