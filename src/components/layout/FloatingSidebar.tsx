...
const BUTTON_CLASS =
  "group flex size-11 items-center justify-center rounded-full border border-white/90 bg-white text-[var(--social-brand-color)] shadow-[0_10px_30px_rgba(2,22,40,0.2)] backdrop-blur-md transition-all duration-200 hover:scale-105 hover:border-[var(--social-brand-color)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/80 focus-visible:ring-offset-2 focus-visible:ring-offset-[#083355]";
...
function getLinksClassName(isExpanded: boolean): string {
  if (isExpanded) {
    return "absolute bottom-full right-0 mb-3 flex flex-col gap-2 transition-all duration-200 translate-y-0 opacity-100";
  }
