...
test("buildTradeMediaPublicUrl encodes every path segment", () => {
  ...
  assert.equal(
    url,
    "/api/trade-media/%E4%BC%97%E5%B2%A9%E8%81%94--%E5%85%83%E7%B4%A0%E5%9B%BE%E6%95%B4%E7%90%86/900x300x9%E7%B3%BB%E5%88%97/%E4%BA%AE%E5%85%89/LV930L902%E7%BB%B4%E5%A4%9A%E5%88%A9%E4%BA%9A.jpg"
  );
});
...
test("resolveTradeMediaPath resolves files inside the trade materials root", () => {
  ...
  assert.equal(
    resolved,
    "/Users/demon/vibecoding/wayon/docs/外贸出口资料/展厅视频/2023-04-15 102614.mov"
  );
});

test("resolveTradeMediaPath blocks path traversal and absolute paths", () => {
  assert.equal(resolveTradeMediaPath(ROOT, ["..", "secret.txt"]), null);
  assert.equal(resolveTradeMediaPath(ROOT, ["/tmp/secret.txt"]), null);
  assert.equal(resolveTradeMediaPath(ROOT, ["%2e%2e", "secret.txt"]), null);
});
